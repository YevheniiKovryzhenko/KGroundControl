...
void JoystickCommandSettings::ArmDisarmParams::load(QSettings &s) {
    s.beginGroup("arm_disarm");
    force = s.value("force", false).toBool();
    s.endGroup();
}
...
namespace remote_control {
    ...
    void manager::emitRoleAssignmentsChanged()
    {
        QList<int> assigned;
    ...
    void manager::subscribeAxisManager(channel::axis::joystick::manager *m)
    {
        if (!m) return;
        ...
        connect(m, &channel::axis::joystick::manager::assigned_value_updated,
                this, [this](int role, qreal val){ 
                    emit roleValueUpdated(role, val); 
                    onRoleValueUpdated(role, val);
                },
                Qt::DirectConnection);
    ...
    void manager::subscribeButtonManager(channel::button::joystick::manager *m)
    {
        if (!m) return;
    ...
    void manager::subscribePovManager(channel::pov::joystick::manager *m)
    {
        if (!m) return;
    ...
    int manager::relayCount() const {
        return m_relayThreads.size();
    }

    JoystickRelayThread* manager::relayThread(int idx) const {
    ...
    bool manager::checkRelayConnectable(int idx)
    {
    ...
    void manager::rewireRelayPort(int idx)
    {
    ...
    void manager::applyConnectability(int idx)
    {
        if (idx < 0 || idx >= m_relayThreads.size()) return;
        while (m_relayCurWiredPort.size() <= idx) m_relayCurWiredPort.append(QString());
    ...
    void manager::onPortsUpdated(const QStringList& ports)
    {
        m_availPorts = ports;
        ...
        for (int i = 0; i < m_commandSettings.size(); ++i)
            applyCommandConnectability(i);
    }

    void manager::onSysidsUpdated(const QVector<uint8_t>& sysids)
    {
    ...
    void manager::onCompidsUpdated(uint8_t sysid,
        const QVector<mavlink_enums::mavlink_component_id>& compids)
    {
        ...
        for (int i = 0; i < m_commandSettings.size(); ++i) {
            if (m_commandSettings[i].sysid == sysid)
                applyCommandConnectability(i);
        }
    }

    void manager::setRelayUserEnabled(int idx, bool enabled)
    {
        if (idx < 0 || idx >= m_relayThreads.size()) return;
        auto *th = m_relayThreads[idx];
        if (!th) return;
    ...
    void manager::rewireRelayPorts()
    {
        for (int i = 0; i < m_relayThreads.size(); ++i)
            applyConnectability(i);
    }

    bool manager::isRelayConnectable(int idx)
    {
        return checkRelayConnectable(idx);
    }

    bool manager::isRelayUserEnabled(int idx)
    {
        if (idx < 0 || idx >= m_relayThreads.size()) return false;
        auto *th = m_relayThreads.at(idx);
        if (!th) return false;
    ...
    QString manager::relayName(int idx) const
    {
    ...
    void manager::updateRelaySettings(int idx, const JoystickRelaySettings& s)
    {
    ...
    void manager::updateRelayFieldRoles(int idx, const QVector<int>& roles)
    {
        if (idx < 0 || idx >= m_relayFieldRoles.size()) return;
    ...
    void manager::updateRelayFieldValues(int idx, const QVector<QString>& vals)
    {
        if (idx < 0 || idx >= m_relayFieldValues.size()) return;
    ...
    void manager::addRelay(const QString& name,
    ...
    void manager::removeRelay(int idx)
    {
        if (idx < 0 || idx >= m_relayThreads.size()) return;
    ...
    int manager::commandCount() const { return m_commandSettings.size(); }

    JoystickCommandSettings remote_control::manager::commandSettings(int idx) const {
        if (idx < 0 || idx >= m_commandSettings.size()) return JoystickCommandSettings{};
        return m_commandSettings.at(idx);
    }
    ...
    void manager::updateCommandSettings(int idx, const JoystickCommandSettings& s) {
        ...
        saveCommands();
        applyCommandConnectability(idx);
    }

    bool manager::isCommandConnectable(int idx) {
        if (idx < 0 || idx >= m_commandSettings.size()) return false;
    ...
    void manager::applyCommandConnectability(int idx) {
        if (idx < 0 || idx >= m_commandSettings.size()) return;
    ...
    void manager::saveCommands() {
        QSettings s;
        s.beginGroup("joystick_manager");
        s.setValue("command/count", m_commandSettings.size());
        for (int i = 0; i < m_commandSettings.size(); ++i) {
            s.beginGroup(QString("command/%1").arg(i));        
            m_commandSettings[i].save(s);         
            s.endGroup();
        }
        s.endGroup();
    }
...
namespace remote_control {
        ...
        namespace axis {
            namespace joystick {
                ...
                void manager::update_value(const int joystick_id, const int axis_id, const qreal value)
                {
                    ...
                    sharedRawValues().setAxisValue(joystick_id, axis_id, calibrated);
        ...
        namespace button {
            namespace joystick {
                ...
                void manager::update_value(const int joystick_id, const int button_id, const bool pressed)
                {
                    ...
                    // qDebug() << "[button::manager] update_value" << joystick_id << button_id << pressed;
                    ...
                    sharedRawValues().setButtonValue(joystick_id, button_id, raw);
    ...
    void manager::run(void)
    {
        ...
        for (int i = 0; i < cmdCount; ++i) {
            ...
            JoystickCommandSettings cs;
            cs.load(s); 
            
            // Ensure it has a UID if loading an old/empty config
    ...
    channel::button::joystick::manager*
